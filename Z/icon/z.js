...
function Zabuton_init(config) { // @arg Object: { asset, iframes }
    ...
    function _watiforAssetIFrameReady() {

        var i = 0, iz = that._config.iframes.length;
        var iAwait = new Await(iz, _iframeReady);

        for (; i < iz; ++i) {
            that._assetIFrames.push(
                    new AssetIFrame({
                        iframe_id:  "iframe" + (i + 1),
                        iframe_src: that._config.iframes[i].iframe_src,
                        worker_src: that._config.iframes[i].worker_src,
                        asset_dir:  that._config.iframes[i].asset_dir,
                        fn:         function() {
                            iAwait.pass();
                        }
                    }));
        ...
        function _iframeReady(err) {
            if (err) { throw err; }

            that._assetIFrames[0].download("16_00", function(err, data, time) {
                if (err) { throw err; }

                console.log("16_00 ready");
/* todo: impl
                //iframe[0].store(id, hash, time, data);
                iframe[0].store("id0", "", time, data);

                alert("download ok: " + url);
 */
            });
            that._assetIFrames[1].download("16_01", function(err, data, time) {
                if (err) { throw err; }

                console.log("16_01 ready");
/* todo: impl
                //iframe[0].store(id, hash, time, data);
                iframe[0].store("id0", "", time, data);
 ...
 */
    ...
    if (global.addEventListener) {
        if (/complete|loaded/.test(document.readyState)) {
            await.pass();
            if (that._config.iframes.length) {
                _watiforAssetIFrameReady();
            }
        } else {
    ...
    if (this._config.iframes.length) {
        ;
    } else {
